...
function Projectile:collitionForEnemy(target)
    if self.x > target.body_x + 100 or target.body_x > self.x + self.width then
        return false
    end

    if self.y > target.body_y + 50 or target.body_y > self.y + self.height then
        return false
    end
...
function Projectile:collitionForSmallEnemy(target)
    if self.x > target.body_x + 50 or target.body_x > self.x + self.width then
        return false
    end
